...
export function pollTokenPrices(maker, token) {
  const CURRENT_PRICE_OSM_IDX = 2;
  const NEXT_PRICE_OSM_IDX = 3;
  ...
  return Promise.all(
    ...
    console.log(`ethereum.js/pollTokenPrices://current of ${token} address ${tokenOsmAddress} is ${currentPrice}`)
    return {
      currentPrice,
      nextPrice
    }
